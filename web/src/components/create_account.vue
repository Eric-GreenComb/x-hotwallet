<template>
  <div class="container register-container" :style="'min-height: '+minHeight">
    ...
    <div class="row"><span class="label">密码：</span><input type="password" v-model="passwd" placeholder="请输入密码"
                                                          class="input"></div>
    <div class="row"><span class="label">交易密码：</span><input type="password" v-model="dealpasswd" placeholder="请输入交易密码"
                                                            class="input"></div>
...
<script>
  ...
  export default {
    ...
    methods: {
      createAccount() {
        let _this = this;
        axios.post(config.create_account+'/'+_this.userid+'/'+_this.name+'/'+_this.passwd).then(function (res) {
