    ...
    <div class="score-container">
      <ul class="score-ul">
        <li class="score-li" v-for="item in data">
          <span><img src="../../static/userhead.jpg" class="icon-score" alt=""></span>
          <span class="score-desc">{{ item.name }}</span>
          <span></span>
          <span @click="addScore(item.address)" class="score-btn"><i class="icon iconfont icon-xinpintuijian"></i></span>
          <div class="clear"></div>
        </li>
      </ul>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addScore(address){
        let _this = this;
        view_create('/13810167616/0x20B5c492B53919e4bfC279C4a43CE350Bb7fDF7c/0x1dd80b503e3b5de5724fe204bc87fb5387b0470c').then(function (res) {
        console.log(res);
          if(res.data.errcode == 0){
            _this.$router.push({
              path: '/wallet',
