<template>
  <div class="container order-create-container" :style="'min-height:'+minHeight">
    ...
    <div class="good disf" v-for="item in cartGoodsChoosed.goods">
      <img :src="item.pic" class="good-pic" alt="">
      <div class="right-div">
        <p class="good-name">{{item.goods_name}}</p>
        <p class="disf size"><span>{{item.price.pvs_name}}</span><span>{{'x' + item.number}}</span></p>
        <p class="price">
          <span>￥</span>
          <span class="price-num" v-if="item.type=='normal'">{{item.price.original_price}}</span>
          <span class="price-num" v-else>{{item.price.promotion_price}}</span>
        </p>
      </div>
    </div>
    <div class="mark">
    ...
    <div class="bgf">
      <p class="disf row">
        <span>商品金额</span>
        <span>{{'￥' + cartGoodsChoosed.total_price}}</span>
      </p>
...
<script>
  import {get_default_address, creat_orders_bycart, get_suit_count, getPersonInfo} from '../../api/index'
  ...
  export default {
    ...
    created() {
      ...
      this.cartGoodsChoosed = JSON.parse(sessionStorage.getItem('cartGoodsChoosed'));
      this.total = this.cartGoodsChoosed.total_price;
      console.log('cartGoodsChoosed', this.cartGoodsChoosed);
    ...
    methods: {
      ...
      submitOrder() {
        ...
        var goodsArr = [];
        _this.cartGoodsChoosed.goods.forEach(function (item) {
        ...
        var obj = {
          ...
          'total_price': _this.cartGoodsChoosed.total_price,//后台传的是商品总价格
          'total_integral': _this.cartGoodsChoosed.total_integral,
          'pay_type': _this.pay_type,
          'voucher_id': _this.voucher_id
        };
        creat_orders_bycart(obj).then(function (res) {
          //console.log('oreder', res);
      ...
      getSuitVoucherCount() {
        var obj = {
          token: localStorage.getItem('token'),
          order_price: this.cartGoodsChoosed.total_price
        };
        var _this = this;
      ...
      useVoucher() {
        ...
        this.$router.push({
          ...
          query: {back: '/main/cart_order_create'}
        })
      },
      //选择收货地址
      chooseAddress() {
        sessionStorage.setItem('remarks', this.remarks);
        this.$router.push('/main/choose_address?back=/main/cart_order_create');
      },
      //获取账户余额
      getAccount() {
        var _this = this;
      ...
      choose_pay_type(type) {
        this.$router.push('/main/pay/' + type + '?back=/main/cart_order_create')
      }
    },
