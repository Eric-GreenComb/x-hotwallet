  ...
  <div class="container order-create-container" :style="'min-height:'+minHeight">
    ...
    <div class="good disf">
      ...
      <div class="right-div">
        ...
        <p class="price"><span>￥</span><span class="price-num">{{goodChoosed.price}}</span></p>
      </div>
    </div>
    <div class="mark">
      <span>备注：</span>
...
<script>
  import {get_default_address, creat_orders, get_suit_count, getPersonInfo} from '../../api/index'
  ...
  export default {
    ...
    methods: {
      ...
      submitOrder() {
        ...
        var inviter, shelves_owner, voucher_id;//邀请人的邀请码、店主（货源）、优惠券id
        if (sessionStorage.getItem('inviter') == 0 || sessionStorage.getItem('inviter') == null || sessionStorage.getItem('inviter') == 'undefined') {
          inviter = '';
        } else {
          inviter = sessionStorage.getItem('inviter');
        }
        if (sessionStorage.getItem('shelves_owner') == 0 || sessionStorage.getItem('shelves_owner') == null || sessionStorage.getItem('shelves_owner') == 'undefined') {
          shelves_owner = '';
        } else {
        ...
        var obj = {
          ...
          'total_price': _this.goodsTotal,//后台传的是商品总价格
        ...
        creat_orders(obj).then(function (res) {
      ...
      useVoucher() {
        ...
        this.$router.push({
          ...
          query: {back: '/main/order_create'}
        })
      },
      //选择收货地址
      chooseAddress() {
        sessionStorage.setItem('remarks', this.remarks);
        this.$router.push('/main/choose_address?back=/main/order_create');
      },
      //获取账户余额
      getAccount() {
        var _this = this;
      ...
      choose_pay_type(type) {
        this.$router.push('/main/pay/' + type + '?back=/main/order_create')
      }
    },
